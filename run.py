...
def items_after_offer_on_coffee(num_of_items):
    """Return number of coffee bags if offer applicable"""
    return num_of_items * 2
...
PRODUCTS_DICT = {
    ...
    "CF1": {
        "name": "Coffee",
        "price": float("11.23"),
        "final_count_method": items_after_offer_on_coffee,
        "final_count": 0
    },
    "MK1": {
        "name": "Milk",
...
def print_final_checkout_price(item_list):
    ...
    for item in final_item_set:
        if PRODUCTS_DICT[item] == "AP1" and offer_applicable:
            # call final_price_per_apple
            final_price_per_item = PRODUCTS_DICT[item]["price_calculation_method"][0]\
                (PRODUCTS_DICT[item]["price"], PRODUCTS_DICT[item]['final_count'])
        ...
        elif PRODUCTS_DICT[item] == "MK1" and offer_applicable:
